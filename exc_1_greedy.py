...
def greedy_qkp(
    ...
    while remaining_capacity > 0:
        ...
        for i in candidates:
            ...
            profits_i = profits[i]
            int_profits_ij = sum(
                quad_profits[i][j] for j in selected
            )
...
def solve_reduced_ilp(
    ...
    S = set(selected)
    R = [
        i for i in range(n) if i not in S
    ]  # set excluding S
    ...
    m.addConstr(
        weight_fixed + sum(weights[i] * x[i] for i in R)
        <= capacity,
        name="capacity",
    )
    ...
    obj = (
        ...
        + sum(
            quad_profits[i][j] * x[j] for i in S for j in R
        )
    )
...
if __name__ == "__main__":
    ...
    for fname in instance_files:
        ...
        results["greedy"].append(
            (fname, len(S), profit_greedy)
        )
        # Run greedy with stopping criterion
        S = greedy_qkp(
            weights, profits, quad, cap, stopping_criterion=5
        )
        # Reduced ILP
        ...
        selected_items = [
            i for i in range(n) if x[i].x > 0.5
        ]
        ilp_profit = compute_profit(
            selected_items, profits, quad
        )
        results["ilp"].append(
            (fname, len(selected_items), ilp_profit)
        )
